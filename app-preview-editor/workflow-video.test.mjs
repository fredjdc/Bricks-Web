...
assert.equal(html.match(/action="https:\/\/bricks\.us11\.list-manage\.com\/subscribe\/post\?u=8254d04472ab0d25110f41edb&amp;id=0bbe1c0355&amp;f_id=00310fe1f0"/g)?.length, 2);
assert.equal(html.match(/name="EMAIL"[^>]*required/g)?.length, 2);
assert.equal(html.match(/name="tags" value="9327311"/g)?.length, 2);
assert.equal(html.match(/name="b_8254d04472ab0d25110f41edb_0bbe1c0355"/g)?.length, 2);
...
assert.equal(html.match(/name="FNAME"[^>]*autocomplete="given-name"[^>]*maxlength="255"/g)?.length, 2);
assert.equal(html.match(/name="LNAME"[^>]*autocomplete="family-name"[^>]*maxlength="255"/g)?.length, 2);
assert.equal(html.match(/name="BUILDING"[^>]*maxlength="255"[^>]*required/g)?.length, 2);
assert.equal(html.match(/name="APPLINK" type="text"[^>]*maxlength="255"/g)?.length, 2);
assert.equal(html.match(/name="CHALLENGE"[^>]*maxlength="255"[^>]*required/g)?.length, 2);
assert.equal(html.match(/First name \(optional\)/g)?.length, 2);
assert.equal(html.match(/Last name \(optional\)/g)?.length, 2);
...
assert.match(script, /for \(const field of \[firstName, lastName\]\)/);
assert.match(script, /const valid = new TextEncoder\(\)\.encode\(field\.value\)\.length <= 255/);
assert.doesNotMatch(script, /Enter your first and last name\./);
assert.match(script, /new TextEncoder\(\)\.encode\(field\.value\)\.length <= 255/);
assert.match(script, /Add a short answer so we can review your application\./);
assert.match(script, /const submitButton = betaStep\.querySelector\('button\[type="submit"\]'\)/);
assert.match(script, /Thanks! Your beta request has been received\. I’ll be in touch by email\./);
assert.match(script, /requestUrl\.pathname = requestUrl\.pathname\.replace\(\/\\\/post\$\/, "\/post-json"\)/);
assert.match(script, /new FormData\(form\)\.forEach/);
assert.match(script, /data\.result !== "success"/);
assert.match(script, /message\.textContent\?\.replace/);
