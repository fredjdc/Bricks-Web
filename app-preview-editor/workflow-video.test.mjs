...
assert.equal(html.match(/name="BUILDING"[^>]*maxlength="255"/g)?.length, 2);
assert.equal(html.match(/name="APPLINK"[^>]*maxlength="255"/g)?.length, 2);
assert.equal(html.match(/name="CHALLENGE"[^>]*maxlength="255"/g)?.length, 2);
...
assert.match(script, /Enter your first and last name\./);
...
assert.match(html, /<script src="script\.js\?v=20260716-3" defer><\/script>/);
