...
window.CalendarScreen = function CalendarScreen({ data, pendingActions, onCreateReservation, onOpenReservation }) {
  const today = window.LIVING_CURRENT_DATE;
  ...
  return (
    <window.FormPanel title={`Nueva política · ${area.name}`} description="Los cambios se aplican desde la fecha indicada. Las reservas existentes conservan sus condiciones." onCancel={onCancel}>
      <form className="living-form" onSubmit={onSubmit}>
        <div className="living-form-section">
          <div className="living-card-label">Identidad y estado</div>
          <div className="living-form-columns">
            <label><span>Nombre</span><input value={values.name} onChange={(event) => update("name", event.target.value)} required /></label>
            <label><span>Ubicación</span><input value={values.location} onChange={(event) => update("location", event.target.value)} required /></label>
          </div>
          <div className="living-form-columns">
            <label><span>Vigente desde</span><input type="date" min={window.LIVING_CURRENT_DATE} value={values.effectiveFrom} onChange={(event) => update("effectiveFrom", event.target.value)} required /></label>
            <label><span>Estado desde vigencia</span><select value={values.status} onChange={(event) => update("status", event.target.value)}><option value="active">Activa</option><option value="closed">Cerrada</option></select></label>
          </div>
          {values.status === "closed" ? <label><span>Mensaje de cierre</span><textarea value={values.closureReason} onChange={(event) => update("closureReason", event.target.value)} placeholder="Motivo opcional visible para administración" /></label> : null}
          <label className="living-check-row"><input type="checkbox" checked={values.reservable} onChange={(event) => update("reservable", event.target.checked)} /><span>Permitir reservas</span></label>
        </div>
